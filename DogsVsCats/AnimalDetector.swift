...
//  DogsVsCats
//
//  Created by Bruno Lopes de Mello on 07/04/19.
...
class AnimalDetector {
    static func startAnimalDetection(_ imageView: UIImageView,
                              completion: @escaping (_ classifications: [String]) -> Void) {
        let imageOrientation = CGImagePropertyOrientation(imageView.image!.imageOrientation)
        let visionRequestHandler = VNImageRequestHandler(cgImage: imageView.image!.cgImage!,
        ...
        let animalDetectionRequest = VNCoreMLRequest(model: catDogModel){(request, error) in
            ...
            guard let observations = request.results else {print("Not results"); return}
